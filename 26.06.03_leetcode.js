...
var earliestFinishTime = function (
    ...
    function help(first, firstDuration, second, secondDuration) {
        let begin = first[0] + firstDuration[0];
        for (let i = 0; i < first.length; i++) {
            begin = Math.min(begin, first[i] + firstDuration[i]);
        }
        let res = begin + second[0] + secondDuration[0];
        for (let i = 0; i < second.length; i++) {
            res = Math.min(res, Math.max(begin, second[i]) + secondDuration[i]);
        }
        return res;
    }
    let land = help(landStartTime, landDuration, waterStartTime, waterDuration);
    let water = help(
        waterStartTime,
        waterDuration,
        landStartTime,
