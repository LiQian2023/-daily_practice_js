...
var earliestFinishTime = function (
    ...
    function getMinTime(first, firstDuration, second, secondDuration) {
        let start = first[0] + firstDuration[0];
        for (let i = 0; i < first.length; i++) {
            start = Math.min(start, first[i] + firstDuration[i]);
        }
        let minTime = start + second[0] + secondDuration[0];
        for (let j = 0; j < second.length; j++) {
            minTime = Math.min(
                minTime,
                Math.max(start, second[j]) + secondDuration[j],
            );
        }
        return minTime;
    }
    const landMinTime = getMinTime(
        landStartTime,
        landDuration,
        waterStartTime,
        waterDuration,
    );
    const waterMinTime = getMinTime(
        waterStartTime,
        waterDuration,
        landStartTime,
        landDuration,
    );
    return Math.min(landMinTime, waterMinTime);
};
