...
var maxNumberOfFamilies = function (n, reservedSeats) {
    const hash = new Map(),
        len = reservedSeats.length;
    for (let i = 0; i < len; i++) {
        const [row, seat] = reservedSeats[i];
        if (seat >= 2 && seat <= 9) {
            if (!hash.has(row)) hash.set(row, 1 << (seat - 2));
            hash.set(row, hash.get(row) | (1 << (seat - 2)));
        }
    }
    let ans = 2 * n;
