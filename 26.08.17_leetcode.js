...
var elevatorRequests = function (n, requests) {
    return requests.reduce((totalTime, request, index) => {
        if (index === 0) {
            // First request, calculate time from floor 0 to the first request
            return totalTime + Math.abs(request - 0);
        } else {
            // Calculate time from the previous request to the current request
            return totalTime + Math.abs(request - requests[index - 1]);
        }
    }, 0);
};
