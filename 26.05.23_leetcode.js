...
function calc(bit, nor, n, data) {
    for (let i = 0; i < n; i++) {
        let x = (data[i][1] >> bit) & 1;
        switch (data[i][0]) {
            case "OR":
                nor |= x;
                break;
...
function main() {
    ...
    let data = [];
    for (let i = 1; i <= n; i++) {
        let [op, t] = input[i].split(" ");
        data.push([op, Number(t)]);
    }
    let ans = 0,
        val = 0;
    for (let i = 29; i >= 0; i--) {
        let bit0 = calc(i, 0, n, data);
        let bit1 = calc(i, 1, n, data);
        if (bit1 > bit0 && val + (1 << i) <= m) {
            ans += 1 << i;
            val += 1 << i;
        } else {
            ans += bit0 << i;
